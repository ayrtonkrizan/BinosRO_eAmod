...
/*==========================================
 * Comando para CorazÛn de Ciudad [Zephyrus]
 *------------------------------------------
 */
...
/*==========================================
 * InformaciÛn de una cuenta [Account ID]
 *------------------------------------------*/
...
/*==========================================
 * CreaciÛn de items en el suelo, en un Area.
 *------------------------------------------*/
ACMD_FUNC(flooritem)
{
	char item_name[100];
...
static int atwhopk_sub(struct block_list *bl,va_list ap)
{
	...
	if( !pl->state.pvpmode || pl == sd )
		return 0; // Ignora jugadores que no estÈn en Modo PK o al Mismo Jugador
...
ACMD_FUNC(mission)
{
	...
	{
		clif_displaymessage(fd, "Voc√™ n√£o est√° ca√ßando .");
		return 0;
	}
	...
	if( c < 1 )
		clif_displaymessage(fd, "Miss√£o de Ca√ßa Completada!");
	else if( sd->hunting_time < (int)time(NULL) )
		clif_displaymessage(fd, "Miss√£o de Ca√ßa Falhada.");
	else
	{
		...
		atcommand_expinfo_sub(tick, &day, &hour, &minute, &second);
		sprintf(atcmd_output, "Tempo Restante: %02d:%02d:%02d", hour, minute, second);
		clif_displaymessage(fd, atcmd_output);
	}
