...
int pc_reg_received(struct map_session_data *sd)
{
	...
	if( battle_config.pc_validate_stats )
	{
		...
		if( u_stats != b_stats )
		{
			...
			sd->status.dex = 1;
			sd->status.luk = 1;
			clif_displaymessage(sd->fd, "Pontos de status anormais encontrados. Foram redefinidos.");
	...
	{
		clif_displaymessage(sd->fd, "Sistema de SeguranÁa Habilitado : Use @security para mais informaÁıes.");
		sd->state.secure_items = 1;
	}
	else
		clif_displaymessage(sd->fd, "Sistema de SeguranÁa Desabilitado : Use @security para mais informaÁıes.");
	...
	if( pc_isPremium(sd) )
	{
		...
		sprintf(output, "Sua conta vip expira em : %d dias, %02d:%02d:%02d", day, hour, minute, second);
		clif_disp_onlyself(sd, output, strlen(output));
	}
