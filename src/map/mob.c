			...
			{ // Hunting Missions [Zephyrus]
				...
				if( pl != NULL && c < 5 && pl->hunting[c].count > 0 )
				{
					...
					if( pl->hunting[c].count > 0 )
						sprintf(output, "[ Missão de Caça : %d '%s' remanescente ]", pl->hunting[c].count, mob_db(md->class_)->jname);
					else
						sprintf(output, "[ Missão de Caça : '%s' Caça Concluída ]", mob_db(md->class_)->jname);
